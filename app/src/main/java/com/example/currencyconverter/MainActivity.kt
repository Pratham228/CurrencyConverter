...
class MainActivity : AppCompatActivity() {
    ...
    private fun getApiResults(){
        if (et_firstConversion!=null && et_firstConversion.text.isNotEmpty() && et_firstConversion.text.isNotEmpty()){
//            val API="https://api.ratesapi.io/api/2010-01-12?base=$baseCurrency&symbols=$convertedToCurrency"
            ...
            if (baseCurrency==convertedToCurrency){
                Toast.makeText(applicationContext,"You have selected same currency",Toast.LENGTH_SHORT).show()
            }else{
